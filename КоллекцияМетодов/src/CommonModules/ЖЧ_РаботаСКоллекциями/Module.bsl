...
Процедура ТипизироватьКолонки(ТаблицаЗначений) Экспорт
	...
	Для Индекс = 0 По КоличествоОбрабатываемыхКолонок - 1 Цикл
		...
		НоваяКолонка = ТаблицаЗначений.Колонки.Добавить(ИмяКолонки, 
			Новый ОписаниеТипов(ТипыЗначенийКолонок[Индекс]), 
			Колонка.Заголовок, Колонка.Ширина);
		
		ТаблицаЗначений.ЗагрузитьКолонку(ЗначенияСтрокКолонок[Индекс], НоваяКолонка);
